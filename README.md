...
**🐳 Plataforma Docker em Produção**

Orquestração com 30+ containers em um Dell PowerEdge: Zabbix, Nextcloud + OnlyOffice, NetBox (IPAM), GLPI (helpdesk), Vaultwarden, UniFi Controller e stack de monitoramento.

`Docker` `Linux` `Caddy` `Let's Encrypt`
...
Deploy IoT para rastreamento e monitoramento de clima: ChirpStack como network server, gateways Dragino, dispositivos Khomp e TrackerD, template Zabbix customizado com 57 itens de monitoramento.

`LoRaWAN` `ChirpStack` `ESP32` `C++`
...
Docker Compose orchestration with 30+ containers on a Dell PowerEdge server: Zabbix, Nextcloud + OnlyOffice, NetBox (IPAM), GLPI (helpdesk), Vaultwarden, UniFi Controller, and monitoring stack.
...
IoT deployment for asset tracking and climate monitoring: ChirpStack as network server, Dragino gateways, Khomp and TrackerD devices, custom Zabbix template with 57 monitoring items.
...
**🐳 Plataforma Docker en Producción**
Orquestación con Docker Compose y más de 30 contenedores en un servidor Dell PowerEdge: Zabbix, Nextcloud + OnlyOffice, NetBox (IPAM), GLPI (helpdesk), Vaultwarden, UniFi Controller y stack de monitoreo.
...
**📡 Infraestructura IoT LoRaWAN**
Despliegue IoT para rastreo de activos y monitoreo climático: ChirpStack como servidor de red, gateways Dragino, dispositivos Khomp y TrackerD, plantilla Zabbix personalizada con 57 ítems de monitoreo.
